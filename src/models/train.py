...
def evaluate_model(model, X_test, y_test, model_name: str) -> dict:
    """
    Scores a fitted model on the test set. Returns a dict of metrics so results
    from multiple models can be compared side by side.

    Using a plain 0.5 threshold here for precision/recall/F1 - actual decision
    thresholds (T1/T2 for approve/review/block) are for later.
    """
...
def train_xgboost(X_train, y_train) -> XGBClassifier:
    # scale_pos_weight roughly balances classes for XGBoost - ratio of
    # negative to positive examples in the training set.
    n_pos = y_train.sum()
...
def main():
    ...
    log_reg = train_logistic_regression(X_train_t, y_train)

    print("Training XGBoost...")
    xgb_model = train_xgboost(X_train_t, y_train)
    ...
    # Pick the winner by PR-AUC - the right metric here given how imbalanced
    # the classes are (plain accuracy or ROC-AUC would be misleading).
